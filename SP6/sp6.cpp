...
bool tcpConnect::sendF(string file){
	...
	int duCo=0;			//count duplicate ACK
	bool rt = FALSE;	//for fast retrans
	...
	for(int cur = 0; cur < fSize; ){
		...
		if((duCo > 3) && (state!=FAST)){
			cout << "Receive three duplicated ACKs." << endl;
			state = FAST;	// => fast recovery
		...
		for(int i = cwnd;i>0;){	//i=unsend in cwnd
			...
			sndPkt.header.urgPtr = rt;//has fast retrans
...
bool tcpConnect::recvF(string file){
	...
	for(int cur = 0; cur < fSize; ){
		...
		else if(rcvPkt.header.seqNum > (ackNum + 1)){
			goto SEND;
			//duplicate ACK
		}
		
		if((rcvPkt.header.seqNum == LOSS) && (rcvPkt.header.urgPtr == 0)){
			//loss packet
			continue;
		}
