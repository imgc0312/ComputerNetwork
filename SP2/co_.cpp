...
char* tcpConnect::getDAddr(){
	return inet_ntoa(outPort.sin_addr);
}

unsigned short tcpConnect::getDPort(){
	return dPort;
}
...
void tcpConnect::freeCon(){		//free connect to any connent
	fp = 0;
	cwnd = 1;				//congestion window
	rwnd = RECV_BUF;		//receive window
	memset((char*)&outPort,0,sizeof(outPort));//the output direct
...
//SP2 ver. if fail would return fail & fix inPort temporarily
bool tcpConnect::waitCon(){	//wait someone make a connect
	...
	//listen to accept
	while(1){
		setTimeOut(0, RTT);	//cancle time out
		if(recvfrom(fd, &rcvPkt, sizeof(rcvPkt), MSG_PEEK, (struct sockaddr*)&outPort, &addrSize) < 0){// PEEK SYN
			//FAIL("receive fail\n");
			continue;
		}

		if(rcvPkt.header.SYN()){ //PEEK SYN packet
			recvfrom(fd, &rcvPkt, sizeof(rcvPkt), 0, (struct sockaddr*)&outPort, &addrSize); //receive
